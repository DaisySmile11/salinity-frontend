...
export async function loadThresholdsFromApi() {
  try {
    const r = await fetch(`${API_BASE}/thresholds`, { cache: "no-store" });
    if (!r.ok) {
      console.warn("loadThresholdsFromApi HTTP", r.status);
      return;
    }
    const t = await r.json();
    if (!t) return;

    // Map DB fields -> local THRESHOLDS keys
    if (typeof t.sal_low === "number") THRESHOLDS.SAL_LOW = t.sal_low;
    if (typeof t.sal_high === "number") THRESHOLDS.SAL_HIGH = t.sal_high;
    if (typeof t.ph_low === "number") THRESHOLDS.PH_LOW = t.ph_low;
    if (typeof t.ph_high === "number") THRESHOLDS.PH_HIGH = t.ph_high;
    if (typeof t.temp_low === "number") THRESHOLDS.TEMP_LOW = t.temp_low;
    if (typeof t.temp_high === "number") THRESHOLDS.TEMP_HIGH = t.temp_high;
    if (typeof t.bat_low === "number") THRESHOLDS.BAT_LOW = t.bat_low;
    if (typeof t.offline_minutes === "number") THRESHOLDS.OFFLINE_MINUTES = t.offline_minutes;
  } catch (e) {
    console.warn("loadThresholdsFromApi failed:", e);
    // Keep defaults, don't crash the whole page
  }
}